...
calandar_effects <- c("dow", "decimal", "week")
...
hypers_param <-
  parameters(penalty(), hidden_units(range = c(10, 150)), epochs(range = c(50, 150))) %>%
...
best_params_topN %>% write.csv(.,"output/dynamic/mlp/best_parmams.csv")
best_params_topN
...
# custom_predict(fit_train$fit,val_data %>% select(-cs))
...
ggsave(
  paste0("output/figures/appendix/figure1d.", DEVICE),
  width = 17,
  height = 11,
  units = "cm",
  dpi = DPI,
...
make_prediction(data = validation_data_lags %>% slice(1:100), model = final_fit_train)
...
intra_val <- compare_forecast_h (
  ...
  save_out = TRUE,
  model_name  = "MLP",
  file = "output/dynamic/forecast_horizon/MLP.csv"
)
toc()
