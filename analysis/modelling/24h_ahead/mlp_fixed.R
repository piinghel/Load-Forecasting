...
# add day after and day before
#holiday_effects <- c(holiday_effects, holiday_effects + days(1), holiday_effects - days(1))
...
hypers_param <-
  parameters(penalty(), hidden_units(range = c(50, 500)), epochs(range = c(50, 250))) %>%
  grid_max_entropy(size = 20)

# visualize the parameters of glmnet
ggplot(hypers_param, aes(x = penalty, y = hidden_units)) +
  geom_point() +
  scale_x_log10()
...
best_params_topN %>% write.csv(.,"output/deterministic/mlp/best_parmams.csv")
best_params_topN
...
predictions_folds <- collect_predictions(results_train) %>%
  inner_join(best_params, by = c("penalty", "hidden_units"))
...
best_params <- tibble(hidden_units = 195, penalty = 4.19e-7, epochs = 76, .config = "Model02")
...
custom_predict <- function(object,
                           newdata) {
  pred <-
    predict(object, as.matrix(newdata)) %>% as.numeric
  return(pred)
}
...
#custom_predict(fit_train$fit,train_data %>% select(-cs))
...
# save for comparison
predictions_val %>% mutate(model = "mlp") %>%
  write.csv("output/deterministic/val_mlp.csv", row.names = FALSE)
...
# save performance
write.csv(pointScore_val,"output/deterministic/mlp/perfPoint_val.csv")
...
plot(vi_val,
     ...
     subtitle = "")
ggsave(
  paste0("output/figures/chapter4/deterministic/mlp/figure1d.", DEVICE),
  width = 17,
  height = 11,
  units = "cm",
  dpi = DPI,
...
prob_val %>% group_by(month,day_type) %>%
...
residuals_grouped <- prob_val %>% group_by(day_type) %>%
...
prob_val <-
  prob_val %>% left_join(y = residuals_grouped, by = c("day_type")) %>%
  mutate(lower_nC = 1,
         upper_nC = 1,
         cover_nC = 1)
...
for (i in 1:nrow(prob_val)) {
  prob_val_bootC[[i]] <- conditional_bootstap1(
    pred = prob_val[i,],
...
write.csv(probScore_val,"output/deterministic/mlp/perfProb_val.csv")
...
# save
probPerf_reliab %>%
  write.csv("output/deterministic/mlp/reliability_val.csv",
            row.names = FALSE)
...
crps_nU <- tibble(
  ...
  dplyr::group_by(hour) %>% dplyr::summarise(value = mean(value)) %>%
...
# save
rbind(probPerf_winkler, probPerf_crsp, probPerf_pinball) %>%
  write.csv("output/deterministic/mlp/probPerf_val.csv",
            row.names = FALSE)
...
# overall
p_reliab <- reliability_diagram(q_hat = rel_b,
                    quantiles = QUANTILES,
                    actuals = prob_val$actual)
...
# save forecasts
prob_val %>%
  write.csv("output/deterministic/mlp/pred_validation.csv",
            row.names = FALSE)
